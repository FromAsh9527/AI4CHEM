#!/bin/bash
# dsub 分片提交 HiTEA Suzuki P4 holdout LOSO（与 amination_v1_full 同款调度）
#
# 用法（在超算 ~/TransferBO2.0 下）:
#   bash scripts/hpc/submit_hitea_p4_dsub.sh
#   DRY_RUN=1 bash scripts/hpc/submit_hitea_p4_dsub.sh
#
# 可调:
...
mkdir -p logs/dsub scripts/hpc/dsub_jobs results/p4_hitea/loso

echo "[INFO] root=$ROOT shards=$N_SHARDS cpu=$JOB_CPU mem_mb=$JOB_MEM_MB"
echo "[INFO] target JSON: 330 under results/p4_hitea/loso/"

for sid in $(seq 0 $((N_SHARDS - 1))); do
  job="scripts/hpc/dsub_jobs/hitea_p4_shard_${sid}.sh"
  out="logs/dsub/hitea_p4_shard_${sid}.out"
  err="logs/dsub/hitea_p4_shard_${sid}.err"
  cat > "$job" <<EOF
#!/bin/bash
#DSUB -n tb2hp4${sid}
#DSUB -N 1
...
python scripts/hpc/run_loso_shard.py \\
  --config configs/hitea_p4_holdout_hpc.yaml \\
  --shard-id ${sid} \\
  --n-shards ${N_SHARDS} \\
  --workers ${JOB_CPU}
...
echo "  find results/p4_hitea/loso -name '*.json' ! -name 'loso_records.json' | wc -l"
echo "  # 目标 330"
echo "汇总:"
echo "  python scripts/run_loso.py --config configs/hitea_p4_holdout_hpc.yaml --rebuild-only"
