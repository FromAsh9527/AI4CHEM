#!/usr/bin/env python
"""Split amination_v1 full LOSO into seed shards for array / dsub jobs.

Example:
  python scripts/hpc/run_amination_v1_full_shard.py --shard-id 0 --n-shards 5 --dry-run
  python scripts/hpc/run_amination_v1_full_shard.py --shard-id $SLURM_ARRAY_TASK_ID --n-shards 5 --workers 16
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "amination_exp_v1_full.yaml",
    )
    args = ap.parse_args()
