#!/bin/bash
# dsub: P1 Suzuki source-subset BO LOSO (360 jobs = 12×2×3×5)
#
#   bash scripts/hpc/submit_p1_suzuki_dsub.sh
#   DRY_RUN=1 bash scripts/hpc/submit_p1_suzuki_dsub.sh
...
DRY_RUN="${DRY_RUN:-0}"
CONFIG="${CONFIG:-configs/suzuki_p1_source_robustness_hpc.yaml}"
OUT_DIR="${OUT_DIR:-results/p1p2_source_robustness/suzuki}"
...
for sid in $(seq 0 $((N_SHARDS - 1))); do
  job="scripts/hpc/dsub_jobs/p1_suzuki_${sid}.sh"
  out="logs/dsub/p1_suzuki_${sid}.out"
  err="logs/dsub/p1_suzuki_${sid}.err"
  cat > "$job" <<EOF
#!/bin/bash
#DSUB -n tb2p1sz${sid}
#DSUB -N 1
...
EOF
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    echo "[SUBMIT] Suzuki P1 shard $sid"
    dsub -s "$job"
  fi
done
