#!/usr/bin/env python
"""Pack P0 shared-init audit for HPC (amination sanity + Suzuki full).
...
INCLUDE = [
    "configs/amination_p0_shared_init_sanity.yaml",
    "configs/suzuki_p0_shared_init_hpc.yaml",
    "src",
    "scripts/run_loso.py",
    "scripts/analyze_p0_shared_init.py",
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "transferbo2_p0_shared_init_hpc.tgz")
    args = ap.parse_args()
    ...
    with tarfile.open(out, "r:gz") as tar:
        for name in (
            "scripts/hpc/submit_p0_amination_sanity_dsub.sh",
            "scripts/hpc/submit_p0_suzuki_dsub.sh",
        ):
    ...
    print("  tar -xzf ~/transferbo2_p0_shared_init_hpc.tgz")
    print("  sed -i 's/\\r$//' scripts/hpc/*.sh")
    print("  bash scripts/hpc/submit_p0_all_dsub.sh")
    return 0
