#!/usr/bin/env python
"""Pack HiTEA Suzuki P4 holdout LOSO tarball for HPC (Windows-friendly).

- Excludes __pycache__ / *.pyc; forces LF for *.sh; normalizes mtime.
...
INCLUDE = [
    "configs/hitea_p4_holdout_hpc.yaml",
    "configs/hitea_p4_pilot.yaml",
    "src",
    "scripts/run_loso.py",
    "scripts/audit_hitea.py",
    "scripts/ingest_hitea.py",
    "scripts/hpc",
    ...
    "data/processed/hitea_long.csv",
    "data/processed/hitea_raw_long.csv",
]
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "transferbo2_hitea_p4_hpc.tgz")
    args = ap.parse_args()
    ...
    with tarfile.open(out, "r:gz") as tar:
        ...
        if not any(n.endswith("transferbo2_hitea.db") for n in names):
            raise SystemExit("hitea DB missing from tarball")
    print(f"[OK] {out}  ({mb:.1f} MB)")
    print("upload, then on HPC:")
    print("  cd ~/TransferBO2.0 && tar -xzf ~/transferbo2_hitea_p4_hpc.tgz")
    print("  bash scripts/hpc/submit_hitea_p4_dsub.sh")
    return 0
