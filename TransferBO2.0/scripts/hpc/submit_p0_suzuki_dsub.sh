#!/bin/bash
# dsub: P0 Suzuki shared-init full (360 jobs = 12×6×5)
#
#   bash scripts/hpc/submit_p0_suzuki_dsub.sh
#   DRY_RUN=1 bash scripts/hpc/submit_p0_suzuki_dsub.sh
...
DRY_RUN="${DRY_RUN:-0}"
CONFIG="${CONFIG:-configs/suzuki_p0_shared_init_hpc.yaml}"
OUT_DIR="${OUT_DIR:-results/suzuki_p0_shared_init}"
...
echo "[INFO] config=$CONFIG  target: 360 under $OUT_DIR/"

for sid in $(seq 0 $((N_SHARDS - 1))); do
  job="scripts/hpc/dsub_jobs/p0_suzuki_${sid}.sh"
  out="logs/dsub/p0_suzuki_${sid}.out"
  err="logs/dsub/p0_suzuki_${sid}.err"
  cat > "$job" <<EOF
#!/bin/bash
#DSUB -n tb2p0sz${sid}
#DSUB -N 1
...
EOF
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    echo "[SUBMIT] suzuki P0 shard $sid"
    dsub -s "$job"
  fi
done
