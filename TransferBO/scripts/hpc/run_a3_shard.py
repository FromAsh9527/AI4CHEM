#!/usr/bin/env python
"""Optional: split A3 into seed shards for Slurm array jobs.

Example:
  # dry-run shard 0/20
  python scripts/hpc/run_a3_shard.py --shard-id 0 --n-shards 20 --dry-run

  # in Slurm array 0-19:
  python scripts/hpc/run_a3_shard.py --shard-id $SLURM_ARRAY_TASK_ID --n-shards 20 --workers 4
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "transfer_grid_edbo_suzuki_a3.yaml",
    )
    args = ap.parse_args()
    if not (0 <= args.shard_id < args.n_shards):
        raise SystemExit(f"shard-id must be in [0, {args.n_shards})")

    # seeds 0..19 → one seed per shard when n_shards=20
    seeds = list(range(20))
