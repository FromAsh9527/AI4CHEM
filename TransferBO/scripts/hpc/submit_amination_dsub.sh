#!/bin/bash
# dsub 分片提交 EDBO amination min S0（与 A3 同款调度）
#
# 用法（在超算 ~/TransferBO 下）:
...
#
# 可调:
#   N_SHARDS=20 JOB_CPU=32 JOB_MEM_MB=65536 ACCOUNT=...
...
mkdir -p logs/dsub scripts/hpc/dsub_jobs results/external_edbo_amination_min_s0

echo "[INFO] root=$ROOT shards=$N_SHARDS cpu=$JOB_CPU mem_mb=$JOB_MEM_MB"
echo "[INFO] target JSON: 2560 under results/external_edbo_amination_min_s0/"

for sid in $(seq 0 $((N_SHARDS - 1))); do
  job="scripts/hpc/dsub_jobs/amin_shard_${sid}.sh"
  out="logs/dsub/amin_shard_${sid}.out"
  err="logs/dsub/amin_shard_${sid}.err"
  cat > "$job" <<EOF
#!/bin/bash
#DSUB -n amins${sid}
#DSUB -N 1
...
echo "  ls results/external_edbo_amination_min_s0/*.json 2>/dev/null | wc -l"
echo "  # 目标 2560"
echo "汇总（回传后本地）:"
echo "  python scripts/summarize_amination_min_s0.py --root results/external_edbo_amination_min_s0"
